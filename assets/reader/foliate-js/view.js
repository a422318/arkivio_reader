...
const isTXT = ({ name, type }) =>
    type === 'text/plain' || name.toLowerCase().endsWith('.txt')
...
export const makeBook = async file => {
    ...
    if (file.isDirectory) {
        ...
        if (await isMOBI(file)) {
            ...
            const { makeTextBook } = await import('./text.js')
...
export class View extends HTMLElement {
    ...
    async open(book) {
        ...
        this.renderer.addEventListener('tap-view', e => this.#emit('tap-view', e.detail))
    ...
    #onRelocate({ reason, range, index, fraction, size, page }) {
        const progress = this.#sectionProgress?.getProgress(index, fraction, size) ?? {}
        const tocItem = this.#tocProgress?.getProgress(index, range)
        const pageItem = this.#pageProgress?.getProgress(index, range)
        const cfi = this.getCFI(index, range)
        this.lastLocation = { ...progress, tocItem, pageItem, cfi, range, page }
